...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def parallel_eval_scenes(self, scene_ids, dump_folder, proc = 2, max_width=0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        for scene_id in scene_ids:
            res_list.append(p.apply_async(self.eval_scene, args=(scene_id, dump_folder), kwds={'max_width': max_width}))
        p.close()
        p.join()
        scene_acc_list = []
        for res in res_list:
    ...
    def eval_seen(self, dump_folder, proc = 2, max_width = 0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        '''
        res = np.array(self.parallel_eval_scenes(scene_ids = list(range(100, 130)), dump_folder = dump_folder, proc = proc, max_width = max_width))
        ap = np.mean(res)
        print('\nEvaluation Result:\n----------\n{}, AP Seen={}'.format(self.camera, ap))
        return res, ap

    def eval_similar(self, dump_folder, proc = 2, max_width = 0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        '''
        res = np.array(self.parallel_eval_scenes(scene_ids = list(range(130, 160)), dump_folder = dump_folder, proc = proc, max_width = max_width))
        ap = np.mean(res)
        print('\nEvaluation Result:\n----------\n{}, AP={}, AP Similar={}'.format(self.camera, ap, ap))
        return res, ap

    def eval_novel(self, dump_folder, proc = 2, max_width = 0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        '''
        res = np.array(self.parallel_eval_scenes(scene_ids = list(range(160, 190)), dump_folder = dump_folder, proc = proc, max_width = max_width))
        ap = np.mean(res)
        print('\nEvaluation Result:\n----------\n{}, AP={}, AP Novel={}'.format(self.camera, ap, ap))
        return res, ap

    def eval_all(self, dump_folder, proc = 2, max_width = 0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        '''
        res = np.array(self.parallel_eval_scenes(scene_ids = list(range(100, 190)), dump_folder = dump_folder, proc = proc, max_width = max_width))
