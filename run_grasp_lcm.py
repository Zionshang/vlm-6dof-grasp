...
class GraspLcmNode:
    ...
    def execute_grasp(self, prompt):
        ...
        print(f"Executing Grasp at {arm_cmd[:3]}...")
        width = sel['width']
        ...
        self.client.set_ee_pose(pre_pose, gripper_pos=self.grip_max, preview_time=1.5)
        time.sleep(2.0)
        ...
        lift_pose[2] += 0.1
        self.client.set_ee_pose(lift_pose, gripper_pos=target_width, preview_time=1.0)
        time.sleep(1.5)
...
def main():
    ...
    parser.set_defaults(prompt="mug") # Default if not overridden, but LCM dictates usually
    ...
    robot_client.set_ee_pose(ready_pose, 0.086, preview_time=1.5)
    time.sleep(2)
